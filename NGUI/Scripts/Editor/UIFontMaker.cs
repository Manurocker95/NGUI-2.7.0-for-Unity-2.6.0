...
public class UIFontMaker : EditorWindow
{
	...
	void OnGUI ()
	{
		...
		{
			NGUISettings.dynamicFont = EditorGUILayout.ObjectField("Font TTF", NGUISettings.dynamicFont, typeof(Font)) as Font;
			...
			NGUIEditorTools.HelpBox("Dynamic fonts require Unity 4.0 or higher.");
		}
		else
		{
NGUISettings.fontData = EditorGUILayout.ObjectField(
    "Font Data", NGUISettings.fontData, typeof(TextAsset)
) as TextAsset;

NGUISettings.fontTexture = EditorGUILayout.ObjectField(
    "Texture", NGUISettings.fontTexture, typeof(Texture2D)
) as Texture2D;
			...
			{
				NGUIEditorTools.HelpBox("The bitmap font creation mostly takes place outside of Unity. You can use BMFont on " +
			...
			{
				NGUIEditorTools.HelpBox("When exporting your font, you should get two files: the TXT, and the texture. Only one texture can be used per font.");
			}
			else if (NGUISettings.atlas == null)
			{
				NGUIEditorTools.HelpBox("You can create a font that doesn't use a texture atlas. This will mean that the text " +
