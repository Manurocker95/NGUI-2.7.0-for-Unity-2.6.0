...
public class UIFontInspector : Editor
{
	...
	public  bool HasPreviewGUI () { return mView != View.Nothing; }
	...
	public override void OnInspectorGUI ()
	{
		...
		if (mType == FontType.Reference)
		{
			...
			NGUIEditorTools.DrawSeparator();
NGUIEditorTools.HelpBox("You can have one font simply point to " +
...
Material mat = EditorGUILayout.ObjectField("Material", mFont.material, typeof(Material)) as Material;
			...
			{
	TextAsset data = EditorGUILayout.ObjectField("Import Data", null, typeof(TextAsset)) as TextAsset;
				if (data != null)
				{
		...
		if (!mFont.isDynamic && mFont.bmFont.isValid)
		{
			if (mFont.atlas != null)
			{
				NGUIEditorTools.DrawHeader("Symbols and Emoticons", true);
				...
				if (symbols.Count == 0)
				{
					NGUIEditorTools.HelpBox("Want to add an emoticon to your font? In the field above type ':)', choose a sprite, then hit the Add button.");
				}
