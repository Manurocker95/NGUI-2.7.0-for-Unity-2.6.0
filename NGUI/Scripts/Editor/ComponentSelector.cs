...
public class ComponentSelector : ScriptableWizard
{
	...
	T o = EditorGUILayout.ObjectField(obj, typeof(T), options) as T;
	...
	static void Show<T> (OnSelectionCallback cb) where T : MonoBehaviour
	{
		...
		ComponentSelector comp = ScriptableWizard.DisplayWizard(
		...
		if (comp != null)
		{
			...
			/*UnityEngine.Object[] loaded = Resources.LoadAll("Objects"); // busca en Assets/Resources/Fonts
			List<MonoBehaviour> objs = new List<MonoBehaviour>();
			for (int i = 0; i < loaded.Length; i++)
			{
				MonoBehaviour mb = loaded[i] as MonoBehaviour;
				if (mb != null) objs.Add(mb);
			}
			comp.mObjects = objs.ToArray();*/
			
			comp.mObjects = UnityEngine.Object.FindObjectsOfTypeAll(type) as MonoBehaviour[];
		}
	...
	void OnGUI ()
	{
		...
		GUILayout.Label("Recently used components");
		NGUIEditorTools.DrawSeparator();

		if (mObjects.Length == 0)
		{
			GUILayout.Box(
    "No recently used " + mType.ToString() + " components found.\n" +
    "Try drag & dropping one instead, or creating a new one.",
