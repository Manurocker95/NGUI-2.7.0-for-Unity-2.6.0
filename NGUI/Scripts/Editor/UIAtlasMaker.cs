...
public class UIAtlasMaker : EditorWindow
{
	...
	/// </summary>
	 ...
	 public static Color32[] GetPixels32(Texture2D tex)
    {
        Color[] cols = tex.GetPixels();
        Color32[] cols32 = new Color32[cols.Length];
	...
	static List<SpriteEntry> CreateSprites (List<Texture> textures)
	{
		...
		foreach (Texture tex in textures)
		{
			...
			Color32[] pixels = GetPixels32(oldTex);
	...
	static void ExtractSprites (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (atlasTex != null)
		{
			...
			foreach (UIAtlas.Sprite asp in list)
			{
				...
				{
					// Read the atlas
					if (oldPixels == null) oldPixels = atlasTex.GetPixels();
    ...
    [MenuItem("WiiGame/Tools/Assign UI Atlas")]
    static void AssignUIAtlas()
    {
        GameObject go = Selection.activeGameObject;
        if (go == null)
        {
            EditorUtility.DisplayDialog("Assign UI Atlas",
                "No GameObject selected.\nPlease select a GameObject in the Hierarchy.",
                "OK");
            return;
        }

        // Add UIAtlas component only if it's not already there
        if (go.GetComponent("UIAtlas") == null)
        {
            go.AddComponent("UIAtlas"); // Unity 2.6: add by string
            EditorUtility.DisplayDialog("Assign UI Atlas",
                "UIAtlas component added to \"" + go.name + "\".",
                "OK");
        }
        else
        {
            EditorUtility.DisplayDialog("Assign UI Atlas",
                "\"" + go.name + "\" already has a UIAtlas component.",
                "OK");
        }
    }
	/// <summary>
...
bool dstr = false;
				   if (Selection.transforms != null && Selection.transforms.Length > 0)
				   {
						for (int i = 0; i < Selection.transforms.Length; ++i)
						{
							...
							go = t.gameObject;
							go.name = NGUISettings.atlasName;
						}
				   }
				   else
				   {
		...
		if (NGUISettings.atlas != null && NGUISettings.atlas.name == NGUISettings.atlasName)
		{
			...
			{
				NGUIEditorTools.HelpBox("You can reveal more options by selecting one or more textures in the Project View window.");
			}
		}
		else
		{
			NGUIEditorTools.HelpBox("You can create a new atlas by selecting one or more textures in the Project View window, then clicking \"Create\".");
		}
