...
//		ti.ReadTextureSettings(settings);
	...
	{
		T[] comps = UnityEngine.Object.FindObjectsOfTypeAll(typeof(T)) as T[];
  ...
  public static void HelpBox(string message)
    {
	 ...
	 public static void DrawTextureWithTexCoords(Rect position, Texture tex, Rect uv, bool alphaBlend)
	{
		if (tex == null) return;

		// Convert UV rect (0..1) to pixel space
		...
		// Clip group so only the UV section shows
		GUI.BeginGroup(position);
		GUI.DrawTexture(
			new Rect(-pixelUV.x, -pixelUV.y, tex.width, tex.height),
			tex,
    ...
    /// <summary>
    /// Unity 2.6 replacement for EditorGUI.DrawPreviewTexture.
    /// Simply draws the texture stretched to the given rect.
    /// The material is ignored because GUI.DrawTexture cannot apply it in 2.6.
    /// </summary>
    public static void DrawPreviewTexture(Rect position, Texture tex, Material mat)
    {
        if (tex == null) return;

        // NOTE: Unity 2.6 GUI.DrawTexture cannot use a Material,
        // so 'mat' parameter is ignored.
        GUI.DrawTexture(
            position,
            tex,
            ScaleMode.StretchToFill,
	...
	public static void DrawSprite (Texture2D tex, Rect rect, Rect outer, Rect inner, Rect uv, Color color, Material mat)
	{
		...
		if (mat == null)
		{
			DrawTextureWithTexCoords(outerRect, tex, uv, true);
		}
		else
		{
			...
			DrawPreviewTexture(outerRect, tex, mat);
		}
		...
		params GUILayoutOption[] options)
	{
		...
		{
			 GUILayout.Box("No sprites found", GUILayout.ExpandWidth(true));
			return;
		}
