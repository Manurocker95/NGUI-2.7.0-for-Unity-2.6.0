...
public class UIPanelInspector : Editor
{
    ...
    {
        // Unity 2.6 fallback: no hay API para comprobar si está abierta; se abre/enfoca.
	...
	public override void OnInspectorGUI ()
	{
		...
		if (!sort)
		{
			NGUIEditorTools.HelpBox("Keep the 'Depth Sort' flag turned on, unless you are working with a UI created prior to NGUI 2.7.0 and want to sort by Z in addition to Depth.", MessageType.Warning);
		}
		...
		if (stat)
		{
			NGUIEditorTools.HelpBox("Only mark the panel as 'static' if you know FOR CERTAIN that the widgets underneath will not move, rotate, or scale. Doing this improves performance, but moving widgets around will have no effect.", MessageType.Warning);
		}
		...
		if (panel.showInPanelTool != EditorGUILayout.Toggle("Panel Tool", panel.showInPanelTool))
		{
			...
			EditorUtility.SetDirty(panel);
			FocusWindowIfItsOpen<UIPanelTool>();
		}
		...
		{
			NGUIEditorTools.HelpBox("Hard clipping has been removed due to major performance issues on certain Android devices. Alpha clipping will be used instead.", MessageType.Warning);
		}

		if (clipping != UIDrawCall.Clipping.None && !NGUIEditorTools.IsUniform(panel.transform.lossyScale))
		{
			NGUIEditorTools.HelpBox("Clipped panels must have a uniform scale, or clipping won't work properly!", MessageType.Error);
		...
		if (panel.drawCalls.size > 0 && NGUIEditorTools.DrawHeader(panel.drawCalls.size + " draw calls from " + panel.widgets.size + " widgets",  "DrawCalls",true))
		{
			NGUIEditorTools.BeginContents();
			
			foreach (UIDrawCall dc in drawcalls)
			{
EditorGUILayout.ObjectField("Material", dc.material, typeof(Material));

				EditorGUILayout.LabelField("Triangles", dc.triangles.ToString());

				if (clipping != UIDrawCall.Clipping.None && !dc.isClipped)
				{
					NGUIEditorTools.HelpBox("You must switch this material's shader to Unlit/Transparent Colored or Unlit/Premultiplied Colored in order for clipping to work.",
						MessageType.Warning);
				}
			}
