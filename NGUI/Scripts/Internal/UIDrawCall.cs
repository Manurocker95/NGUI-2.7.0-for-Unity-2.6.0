...
public class UIDrawCall : MonoBehaviour
{
	...
	public void Set (BetterList<Vector3> verts, BetterList<Vector3> norms, BetterList<Vector4> tans, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			if (verts.size < 65000)
			{
				...
				Color32[] col32 = cols.ToArray();
				Color[] col = new Color[col32.Length];
				for (int i = 0; i < col32.Length; i++)
					col[i] = col32[i]; // conversión implícita Color32 -> Color
	...
	void OnWillRenderObject ()
	{
		...
		if (mMat != null && isClipped)
		{
			...
			Vector2 sharpness2 = new Vector2(1000.0f, 1000.0f);
			if (mClipSoft.x > 0f) sharpness2.x = mClipRange.z / mClipSoft.x;
			if (mClipSoft.y > 0f) sharpness2.y = mClipRange.w / mClipSoft.y;

			// Expand to Vector4 because Unity 2.6 Material.SetVector requires Vector4
			Vector4 sharpness = new Vector4(sharpness2.x, sharpness2.y, 0f, 0f);
