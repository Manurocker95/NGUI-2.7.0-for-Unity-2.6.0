...
public abstract class UIWidget : MonoBehaviour
{
	...
	{ 
		...
		{ 
			if (mTrans == null) 
			{
				...
				m_LastRotation = cachedTransform.rotation;
				m_LastScale = cachedTransform.localScale;		
			}
			return mTrans; 
		} 
	}
	...
	{
		 if (cachedTransform.position != m_LastPosition ||
			cachedTransform.rotation != m_LastRotation ||
			cachedTransform.localScale != m_LastScale)
		{
			m_LastPosition = cachedTransform.position;
			m_LastRotation = cachedTransform.rotation;
			m_LastScale = cachedTransform.localScale;
	...
	virtual public Vector2 relativeSize { get { return new Vector2(1,1); } }
