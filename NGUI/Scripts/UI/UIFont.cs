...
public class UIFont : MonoBehaviour
{
...
#if DYNAMIC_FONT
		if (a.isDynamic && b.isDynamic && a.dynamicFont != null && b.dynamicFont != null)
		{
			if (a.dynamicFont.name == b.dynamicFont.name)
				return true;
		}
#endif
	...
	public void Request (string text)
	{
		if (!string.IsNullOrEmpty(text))
		{
			...
			{
				VP_Utils.RequestCharactersInTexture(mDynamicFont, "j", mDynamicFontSize, mDynamicFontStyle);
				VP_Utils.GetCharacterInfo(mDynamicFont, 'j', out mTemp, mDynamicFontSize, mDynamicFontStyle);
				mDynamicFontOffset = (mDynamicFontSize + mTemp.vert.yMax);
				VP_Utils.RequestCharactersInTexture(mDynamicFont, text, mDynamicFontSize, mDynamicFontStyle);
			}
		}
	}
#endif
	...
	public Vector2 CalculatePrintedSize (string text, bool encoding, SymbolStyle symbolStyle)
	{
...
#endif
		{
...
#if DYNAMIC_FONT
			if (mDynamicFont != null) VP_Utils.RequestCharactersInTexture(mDynamicFont, text, mDynamicFontSize);
#endif
			...
			for (int i = 0; i < length; ++i)
			{
				...
				{
					if (VP_Utils.GetCharacterInfo(mDynamicFont, c, out mChar, mDynamicFontSize, mDynamicFontStyle))
	...
	public string GetEndOfLineThatFits (string text, float maxWidth, bool encoding, SymbolStyle symbolStyle)
	{
		...
		while (currentCharacterIndex > 0 && remainingWidth > 0)
		{
			...
			{
				if (VP_Utils.GetCharacterInfo(mDynamicFont, currentCharacter, out mChar, mDynamicFontSize, mDynamicFontStyle))
	...
	public bool WrapText (string text, out string finalText, float width, float height, int lines, bool encoding, SymbolStyle symbolStyle)
	{
		...
		for (; offset < textLength; ++offset)
		{
			...
			{
				if (VP_Utils.GetCharacterInfo(mDynamicFont, ch, out mChar, mDynamicFontSize, mDynamicFontStyle))
		...
		bool encoding, SymbolStyle symbolStyle, Alignment alignment, int lineWidth, bool premultiply)
	{
		...
		else if (text != null)
		{
			...
			int fs = size;
			Vector2 invSize = fs > 0 ? new Vector2(1f / fs, 1f / fs) : new Vector2(1,1);
